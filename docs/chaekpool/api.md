...
- **환경변수 관리**: Jenkins Secret file credential (`api-env-file`)에서 관리. 변경 시 Jenkins UI → Credentials에서 수정 후 파이프라인 재실행
...
> **시크릿 관리**: Jenkins UI → Manage Jenkins → Credentials → `api-env-file`에서 직접 수정 가능.
...
| `service/chaekpool/ansible/roles/jenkins/templates/casc.yaml.j2` | JCasC (api-env-file credential) |
