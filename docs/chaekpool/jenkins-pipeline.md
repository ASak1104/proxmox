...
- **빌드 환경**: Docker 컨테이너 (`gradle:jdk25-alpine`, `-u 1000:1000 --group-add 104` 실행 — 베이스 이미지 `gradle` 유저(UID 1000) 및 호스트 `jenkins-agent`(UID 1000) 양쪽과 정합. Jenkins 공식 이미지 컨벤션과 일치)
...
- 원인: 컨테이너 실행 UID와 호스트 `jenkins-agent` UID 불일치 → 워크스페이스 산출물이 타 사용자 소유로 남음
- 확인: `ssh root@10.1.0.130 "find /var/lib/jenkins-agent/workspace ! -user jenkins-agent | head"`
- 일회성 복구: `ssh root@10.1.0.130 "chown -R jenkins-agent:jenkins-agent /var/lib/jenkins-agent/workspace"`
- 항구 대책: 호스트 `jenkins-agent` UID/GID = 1000 + `ci/Jenkinsfile`의 docker agent `args`에 `-u 1000:1000 --group-add 104` 유지 (베이스 이미지 `gradle:jdk25-alpine`의 `gradle` 유저 UID와 동일)
